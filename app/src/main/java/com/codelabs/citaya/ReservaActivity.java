...
public class ReservaActivity extends AppCompatActivity {
    ...
    // Nombre del doctor que debe mostrarse SIEMPRE aunque esté "completo",
    // porque es la propia cita que se está reprogramando.
    ...
    // Lista de doctores que SÍ se muestran en pantalla (ya filtrados por disponibilidad)
    ...
    protected void onResume() {
        super.onResume();
        // Refresca los contadores por si, en otra pantalla, se liberó o se llenó
        // el cupo de algún doctor.
        actualizarContadoresEspecialidades();
    }
    ...
    private void configurarModoReprogramar() {
        ...
        if (modoReprogramar) {
            ...
            // El doctor de la cita que se está moviendo debe verse siempre,
            // aunque sus 9 horarios estén ocupados por otras citas.
    ...
    // ================== NUEVA LÓGICA DE DISPONIBILIDAD POR DOCTOR ==================

    /**
     * Las 9 horas fijas del día, leídas directamente de los TextView de la UI
     * (09:00...18:00), para no duplicar esos valores como texto suelto.
     */
    ...
    /**
     * Un doctor está "completamente ocupado" cuando, para las 9 horas fijas,
     * no queda ninguna sin al menos 1 cita activa (en cualquier fecha).
     */
    private boolean doctorCompletamenteOcupado(String doctorNombre) {
        int medicoId = medicoDAO.obtenerIdPorNombre(doctorNombre);
        if (medicoId == -1) return false; // si no lo encontramos en BD, no lo ocultamos

        String[] horasFijas = obtenerHorasFijas();
        boolean tieneHoraLibre = reservaDAO.medicoTieneHoraLibre(medicoId, horasFijas);
        return !tieneHoraLibre;
    }

    /**
     * Cuenta cuántos doctores de una especialidad tienen al menos 1 horario libre.
     */
    ...
    /**
     * Actualiza el texto "X doctores disponibles" de cada card de especialidad.
     * Estructura asumida (según tu XML): card -> [icono, contenedorTextos[titulo, subtitulo]]
     */
    ...
    private void actualizarContadorCard(LinearLayout card, String especialidad) {
        ...
        try {
            ...
            if (disponibles <= 0) {
                subtitulo.setText("Sin disponibilidad por ahora");
            } else if (disponibles == 1) {
                subtitulo.setText("1 doctor disponible");
            } else {
                subtitulo.setText(disponibles + " doctores disponibles");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void abrirDoctores(String especialidad) {
        ...
        if (disponibles.isEmpty()) {
            Toast.makeText(this,
                    "En este momento no hay doctores disponibles para " + especialidad,
    ...
    private void seleccionarDoctor(int d) {
        ...
        int index = d - 1;
        if (doctoresVisiblesActuales == null || index < 0 || index >= doctoresVisiblesActuales.size()) {
            return;
        }
        ...
        if (cards[index] != null) {
            cards[index].setBackgroundResource(R.drawable.bg_card_selected);
        }
        if (checks[index] != null) {
            checks[index].setVisibility(View.VISIBLE);
        }
        ...
        try {
            ...
            if (horaLimpia.contains("AM") || horaLimpia.contains("PM")) {
                java.text.SimpleDateFormat formato12 =
                        new java.text.SimpleDateFormat("d/M/yyyy hh:mm a", Locale.US);
    ...
    private void confirmarCita() {
        ...
        // Verificar si el usuario ya tiene una cita ese mismo día y hora
        boolean yaTieneCita = reservaDAO.usuarioTieneCitaEnHorario(
                usuarioId,
                fecha,
                horaSeleccionada
        );
        ...
        if (yaTieneCita) {
            Snackbar.make(
                    ...
                    "Ya tienes una cita programada para esa fecha y hora.",
                    Snackbar.LENGTH_LONG
            ).show();
            return;
        }
        // 2. Doctor ya tiene paciente en esa hora
        boolean doctorOcupado = reservaDAO.doctorTieneCita(
                medicoId,
                fecha,
                horaSeleccionada
        );
    ...
    private void liberarHorario(String doctor, String fecha, String horaAMPM) {

        SharedPreferences prefsGlobal = prefsHorariosGlobales();

        Set<String> ocupados =
                prefsGlobal.getStringSet("ocupados", new HashSet<>());

        Set<String> copia = new HashSet<>(ocupados);

        String hora24 = convertirHora24(horaAMPM);

        String clave = doctor + "|" + fecha + "|" + hora24;

        copia.remove(clave);

        prefsGlobal.edit()
                .putStringSet("ocupados", copia)
                .apply();
    }
    ...
    private String convertirHora24(String horaAMPM) {
        try {
            java.text.SimpleDateFormat formato12 =
                    new java.text.SimpleDateFormat("hh:mm a", Locale.US);
            java.text.SimpleDateFormat formato24 =
