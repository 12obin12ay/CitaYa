...
public class EspecialidadesActivity extends AppCompatActivity {
    ...
    private int contarDoctoresDisponibles(String especialidad) {
        ...
        for (Doctor d : doctores) {
            if (!doctorCompletamenteOcupado(d.nombre)) disponibles++;
        }
        return disponibles;
    }
    ...
    private void abrirDoctores(String especialidad) {
        ...
        for (Doctor d : todos) {
            if (!doctorCompletamenteOcupado(d.nombre)) disponibles.add(d);
        }
    ...
    private boolean horaYaPaso(String fecha, String horaTexto) {
        try {
            ...
            java.text.SimpleDateFormat format = (horaLimpia.contains("AM") || horaLimpia.contains("PM")) ? 
                new java.text.SimpleDateFormat("d/M/yyyy hh:mm a", Locale.US) : 
                new java.text.SimpleDateFormat("d/M/yyyy HH:mm", Locale.US);
            java.util.Date fechaHora = format.parse(fecha + " " + horaLimpia);
            return fechaHora != null && fechaHora.before(new java.util.Date());
    ...
    private void confirmarCita() {
        ...
        // 🔥 CORREGIDO: Ahora enviamos los 9 parámetros requeridos por el constructor de Reserva
