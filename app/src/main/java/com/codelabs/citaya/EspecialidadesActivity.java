...
public class EspecialidadesActivity extends AppCompatActivity {
    ...
    private SharedPreferences prefsHorariosGlobales() {
        return getSharedPreferences("HORARIOS_GLOBALES", MODE_PRIVATE);
    }
    ...
    private void confirmarCita() {
        ...
        Set<String> ocupadosGlobales = prefsGlobal.getStringSet("ocupados", new HashSet<>());
        Set<String> copiaOcupadosGlobales = new HashSet<>(ocupadosGlobales);

// La clave ahora incluye doctor + fecha + hora
        String clave = doctorSeleccionado + "|" + fecha + "|" + horaSeleccionada;

        if (copiaOcupadosGlobales.contains(clave)) {
            Snackbar.make(findViewById(android.R.id.content),
                    "Este horario ya está ocupado para este doctor",
...
// Guardamos la cita del usuario
        prefs.edit()
                .putStringSet("lista", nuevaLista)
                .apply();

// Guardamos el horario ocupado para TODOS los usuarios
        copiaOcupadosGlobales.add(clave);

        prefsGlobal.edit()
                .putStringSet("ocupados", copiaOcupadosGlobales)
                .apply();
